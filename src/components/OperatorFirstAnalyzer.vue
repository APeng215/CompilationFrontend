...
export default defineComponent({
  ...
  methods: {
    request() {
      fetch(Config.endPointIP + "/compilation/api/operatorFirstAnalysis", {
        method: "POST",
        body: this.rowString,
        headers: {
          ...
          .then((json) => {
            this.isSentenceCorrect = json.isSentenceCorrect;
            this.firstvtSet = json.firstvtSet;
            this.lastvtSet = json.lastvtSet;
            this.priorityTable = json.priorityTable;
            this.analysisResult = json.analysisResult;
            console.log(json)
          })
    }
  }
    ...
    <div id="parent-right" class="allCenter shadow">
      ...
      <el-scrollbar class="scrollbar">
        <div id="right" class="allCenter">
          ...
          <span class="arg">被分析语句是否正确</span>
          <el-input
              class="output"
              type="textarea"
              :autosize="{ minRows: 2 }"
              placeholder=""
              v-model="isSentenceCorrect"
          >
          </el-input>

          <span class="arg">FIRSTVT集</span>
          <el-input
              class="output"
              type="textarea"
              :autosize="{ minRows: 2 }"
          ...
          <span class="arg">算符优先分析表</span>
          <el-input
              class="output"
              type="textarea"
              :autosize="{ minRows: 2 }"
              placeholder=""
              v-model="priorityTable"
          >
          </el-input>
